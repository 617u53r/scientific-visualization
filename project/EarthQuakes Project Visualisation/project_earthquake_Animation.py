...
def main():
    ...
    # A 3D visualisation showing some glyph at longitude, latitude and depth.
    #  The strength (often between 1.0-6.0) should affect the glyph (size, colour or your own
    # choice.) Are spheres really the best choice?
...
class KeyboardInterface(object):
    ...
    def keypress(self, obj, event):
        ...
        while self.current_time < self.max_time:
            ...
            # self.window2image_filter.SetInputBufferTypeToRGBA()
            # self.window2image_filter.ReadFrontBufferOff()
            # self.window2image_filter.Update()
            # writer = vtk.vtkAVIWriter()
            # writer.SetInputConnection(self.window2image_filter.GetOutputPort())
            # writer.SetFileName("project.avi")
            # writer.Start()



# Make a reader that reads the data that you download from the website. You can chose
# any time window and any region. But it is recommended to use the latest data going
# back to the earthquake of 24 August (at 3.36, with magnitude 6.0).
def parse_csv(path):
    """
...
def render_3D_points(location, magnitude, time_seconds, data):
    ...
    planeMapper.SetInputConnection(
        plane.GetOutputPort())  # polydatamapper divides plane data into points, lines or triangles
    ...
    magnitudetext.SetInput("SeSe Visualization Project")
    ...
    render_window.SetSize(1500, 1200)
